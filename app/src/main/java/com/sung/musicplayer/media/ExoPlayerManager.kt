...
class ExoPlayerManager(val context: Context) : OnExoPlayerManagerCallback {
    ...
    private fun tryToGetAudioFocus() {
        ...
        mCurrentAudioFocusState =
            if (result == AudioManager.AUDIOFOCUS_REQUEST_GRANTED) AUDIO_FOCUSED else AUDIO_NO_FOCUS_NO_DUCK
    }

    override fun play(aSong: Song) {
        mPlayOnFocusGain = true
        tryToGetAudioFocus()
        registerAudioNoisyReceiver()

        val songHasChanged = aSong.id != mCurrentSong?.id
        if (songHasChanged)
            mCurrentSong = aSong

        if (songHasChanged || mExoPlayer == null) {
            releaseResources(false) // release everything except the player
            val source = mCurrentSong?.path
            if (mExoPlayer == null) {
